...
%   - two calls to function PDE_solver that solves Eqs (3)-(5) using 
%   Newton-Raphson method with one of the two possible initial conditions:
%   compact support as in Eqs (6)-(7) or exponential decay front
%   as in Eqs (8)-(9). The function displays time-dependent
...
% Function PDE_solver
% This function solves Eqs (3)-(5) using Newton-Raphson method 
% with one of the two possible initial conditions:
% compact support as in Eqs (6)-(7) or exponential decay front
% as in Eqs (8)-(9). The function displays time-dependent
% solutions u(x,t) and s(x,t) at requested times.
% INPUT ARGUMENTS:
% -- r1 and r2: parameters r1 and r2 from Eq (4), such as r1 > 0 and r2 > 0
% -- decayrate: if the initial condition used include a front with an
% exponential decay, decayrate represents the exponential decay rate a as in
% Eq (8). If the initial condition used is on compact support, decayrate
% must be set to Inf
